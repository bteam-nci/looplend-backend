...
module.exports.listMessages = async (params, dbInstance) => {
	...
	const query = dbInstance("conversation_messages").where("rentalId", rID).innerJoin("users", "users.id", "conversation_messages.senderId").select("conversation_messages.*", "users.fullName as senderName");
